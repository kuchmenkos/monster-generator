...
import type { SaveV1 } from "../game/state";
...
<header class="shell-header">
  ...
  <div class="shell-meta">
    ...
    <button class="hype-chip" id="hype-chip" title="Хайп">${icon("Zap")} <span id="hype-value">0</span></button>
    <button class="box-button" id="box-button">${icon("Package")}<span class="box-label">Бокс · ${BOX_HYPE_COST}</span></button>
  </div>
</header>
...
<dialog id="modal"><div class="modal-heading"><h2 id="modal-title"></h2><button id="close-modal" class="icon-button" aria-label="Закрыть">${icon("X")}</button></div><div id="modal-body"></div></dialog>`;
  ...
  host.querySelector("#close-modal")!.addEventListener("click", () =>
    api.closeModal(),
  );
